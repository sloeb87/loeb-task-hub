...
SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname = 'create_all_recurring_instances';

-- Then recreate the trigger function with proper array handling for ALL scenarios
...
BEGIN
  ...
  IF NEW.is_recurring = true AND NEW.recurrence_end_date IS NOT NULL AND NEW.recurrence_type IS NOT NULL THEN
    ...
    IF OLD IS NULL OR OLD.is_recurring = false OR 
      ...
      -- CRITICAL: Convert all potential scalar values to proper arrays
      -- Handle scope - must always be an array, never null
      IF NEW.scope IS NULL THEN
        safe_scope := ARRAY[]::text[];
      ELSIF pg_typeof(NEW.scope) = 'text'::regtype THEN
        safe_scope := ARRAY[NEW.scope::text];
      ELSE
        safe_scope := NEW.scope;
      END IF;
      
      -- Handle stakeholders - can be null or empty array
      IF NEW.stakeholders IS NULL THEN
        safe_stakeholders := ARRAY[]::text[];
      ELSIF pg_typeof(NEW.stakeholders) = 'text'::regtype THEN
        safe_stakeholders := ARRAY[NEW.stakeholders::text];
      ELSE
        safe_stakeholders := NEW.stakeholders;
      END IF;
      
      -- Handle dependencies - can be null or empty array  
      IF NEW.dependencies IS NULL THEN
        safe_dependencies := ARRAY[]::text[];
      ELSIF pg_typeof(NEW.dependencies) = 'text'::regtype THEN
        safe_dependencies := ARRAY[NEW.dependencies::text];
      ELSE
        safe_dependencies := NEW.dependencies;
      END IF;
      
      -- Handle recurrence_days_of_week - must be integer array or empty
      IF NEW.recurrence_days_of_week IS NULL THEN
        safe_recurrence_days := ARRAY[]::integer[];
      ELSIF pg_typeof(NEW.recurrence_days_of_week) = 'integer'::regtype THEN
        safe_recurrence_days := ARRAY[NEW.recurrence_days_of_week::integer];
      ELSE
        safe_recurrence_days := NEW.recurrence_days_of_week;
      END IF;
      ...
      LOOP
        ...
        INSERT INTO public.tasks (
          ...
          ARRAY[]::integer[] -- Clear days of week for instances
        );
