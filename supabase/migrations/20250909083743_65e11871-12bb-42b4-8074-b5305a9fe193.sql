-- Review and update all backend logic to use Task ID instead of UUID

-- 1. First, let's update the follow_ups table to use task_number instead of UUID
ALTER TABLE public.follow_ups 
ADD COLUMN task_number text;

-- Update existing follow_ups to use task_number
...
-- Make task_number NOT NULL after population
ALTER TABLE public.follow_ups 
ALTER COLUMN task_number SET NOT NULL;

-- Drop the old task_id column and rename task_number to task_id
ALTER TABLE public.follow_ups 
DROP COLUMN task_id;
...
-- 2. Update task_metrics table to use task_number as primary reference
-- The task_metrics table already has task_number, so we just need to update the logic

-- 3. Update all functions to use task_number instead of UUID
...
-- Update RLS policies for follow_ups to work with task_number
